...
import { useDataPanels, useGroupRanks, useSymbolQuote } from "@/lib/api";
  ...
  useEffect(() => {
    async function fetchRelatedSymbols() {
      try {
        ...
        const result = await queryScanner<Symbol>("symbols", {
          columns: [
            "ticker",
            "logo",
            "name",
...
export function DataPanel({
  ...
  const ticker = useSymbol();
  const { data: symbolData, isLoading, error } = useSymbolQuote(ticker);
