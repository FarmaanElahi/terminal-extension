...
export type Param = GlobalFeedParam | SymbolFeedParam;

export async function fetchStockTwit(params: Param): Promise<StockTwitFeed> {
  ...
  if (!response.ok) {
    console.error("StockTwit fetch failed", response.statusText);
    throw new Error(`Failed to fetch StockTwit data: ${response.status}`);
  }
